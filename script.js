...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function setupScrollReveals() {
    ...
    // Standard Projects cards reveals (staggered)
    const standardProjectCards = document.querySelectorAll('.projects-grid .project-card');
    if (standardProjectCards.length > 0) {
      gsap.from(standardProjectCards, {
        scrollTrigger: {
  ...
  // Bulletproof fallback: Force visibility of all cards after 3.5 seconds in case scroll triggers fail
  setTimeout(() => {
    const hiddenElements = document.querySelectorAll('.skills-card, .project-card, .section-header, .about-text-card, .technext-card');
    hiddenElements.forEach(el => {
      const opacity = window.getComputedStyle(el).opacity;
      if (opacity === '0' || opacity === 0 || opacity === '') {
        gsap.to(el, { opacity: 1, y: 0, x: 0, duration: 0.5, ease: 'power2.out', overwrite: 'auto' });
      }
    });
  }, 3500);
});
